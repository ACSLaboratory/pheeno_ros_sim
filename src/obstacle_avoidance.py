...
def obstacle_check(msg, ir_location):
    ...
    if msg.ranges[0] < sensor_limits[ir_location]:
...
if __name__ == "__main__":
    ...
    sub_ir_center = rospy.Subscriber(
        pheeno_number + "/scan_center", LaserScan, obstacle_check,
        callback_args="center")
    sub_ir_back = rospy.Subscriber(
        pheeno_number + "/scan_back", LaserScan, obstacle_check,
        callback_args="back")
    sub_ir_right = rospy.Subscriber(
        pheeno_number + "/scan_right", LaserScan, obstacle_check,
        callback_args="right")
    sub_ir_left = rospy.Subscriber(
        pheeno_number + "/scan_left", LaserScan, obstacle_check,
        callback_args="left")
    sub_ir_cr = rospy.Subscriber(
        pheeno_number + "/scan_cr", LaserScan, obstacle_check,
        callback_args="cr")
    sub_ir_cl = rospy.Subscriber(
        pheeno_number + "/scan_cl", LaserScan, obstacle_check,
        callback_args="cl")
    ...
    sensor_limits = {"center": 0.3, "cr": 0.3, "right": 0.3,
